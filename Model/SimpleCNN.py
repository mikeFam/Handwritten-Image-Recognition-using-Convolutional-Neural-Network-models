...
# Declare convolution layers:
class SimpleCNN_net(nn.Module):
    def __init__(self):
        super(SimpleCNN_net, self).__init__()
    ...
    # In this simple CNN model, we decide to use ReLU activation function after every
    def forward(self, x):
        x = self.conv1(x)
        x = F.relu(x)
        x = self.maxpool1(x)
